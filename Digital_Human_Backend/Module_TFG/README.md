...
----------
 ...
 - **Cache mode to store face detection results to eliminate face detection process from the second attempt for the same video**
 - **Multiplier argument added to accelerate the process speed**
 - **Fix fps argument to make functional properly**
...
- Install `PyTorch` using the instructions given on the official site. E.g. for CUDA 11.8 with pip on Windows, you can use `pip3 install torch --index-url https://download.pytorch.org/whl/cu118`.
...
```bash
python inference.py --checkpoint_path ./input_model/wav2lip.pth --face input_video/1_bbaf2n.mp4 --audio input_audio/audio.wav --multiplier 1
```
