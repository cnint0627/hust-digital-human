...
class StyleLoader(torch.utils.data.Dataset):
    def __init__(self, file_paths: list, hparams):
        self.audio_paths = []
        for file_path in file_paths:
            with open(f'../../datasets/emotions/{file_path}/metadata.csv') as f:
                for line in f:
                    self.audio_paths.append(f'../../datasets/emotions/{file_path}/wavs/{line.strip().split("|")[0]}')

        self.sampling_rate = hparams.sampling_rate
        self.max_wav_value = hparams.max_wav_value
        # shuffle(self.audio_paths)
        # print(self.audio_paths[:10])
    ...
    def __getitem__(self, idx):
        ...
        if sampling_rate != self.sampling_rate:
            ...
            #         idx, sampling_rate, self.sampling_rate
            #     )
            # )
            audio = torch.FloatTensor(librosa.resample(y=audio.numpy(), orig_sr=sampling_rate, target_sr=self.sampling_rate))
...
class TextAudioCollate():
    ...
    def __call__(self, batch):
        ...
        return text_padded, text_lengths, bert_padded, spec_padded, spec_lengths, wav_padded, wav_lengths, style
