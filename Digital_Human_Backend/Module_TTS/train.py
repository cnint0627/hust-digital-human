...
import commons
import utils
from data_utils import TextAudioLoader, TextAudioCollate, DistributedBucketSampler, StyleLoader
...
def run(rank, n_gpus, hps):
    ...
    try:
        ...
        epoch_str = 1
        global_step = 0
        logger.info(f'model not exists: {e}')
...
def train_and_evaluate(
    rank, epoch, hps, nets, optims, schedulers, scaler, loaders, logger, writers, style_dataset
):
    ...
    if rank == 0:
        ...
        loader = train_loader
    for batch_idx, (x, x_lengths, bert, spec, spec_lengths, y, y_lengths, style) in enumerate(loader):
        """
            x: text_padded
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            with autocast(enabled=False):
                ...
                loss_mel = F.l1_loss(y_mel - torch.mean(y_mel, dim=2, keepdim=True),
                                     y_hat_mel - torch.mean(y_hat_mel, dim=2, keepdim=True)) * hps.train.c_mel
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                logger.info(
                    f"loss_kl_r={loss_kl_r:.3f}, loss_F0={loss_F0: .3f}, loss_style={loss_style:.3f}"
                )
...
def evaluate(hps, generator, eval_loader, writer_eval):
    generator.eval()
    with torch.no_grad():
        for batch_idx, (x, x_lengths, bert, spec, spec_lengths, y, y_lengths, style) in enumerate(eval_loader):
